...
let leaveRecords = JSON.parse(localStorage.getItem(LEAVE_KEY) || "[]");
...
function saveLeave() {
    localStorage.setItem(LEAVE_KEY, JSON.stringify(leaveRecords));
}
...
function generateID() {
    ...
    const numbers = employees
        .map(emp => {
            const match = String(emp.id || "").match(/^EMP(\d+)$/);
...
function getMonthSalaryPayment(employeeID, month) {
...
function getSalaryDetails(employee, month) {
...
/* =====================================================
   IMPORTANT LEAVE CHECK
===================================================== */

/*
   LEAVE LOGIC

   1. Employee ID is compared as a STRING.
      This prevents EMP003 vs 003 type problems.

   2. If a leave record has NO dates:
      It is considered ACTIVE / ON LEAVE.

   3. If only START DATE exists:
      Employee is on leave from that date onward.

   4. If only END DATE exists:
      Employee is on leave until that date.

   5. If both dates exist:
      The leave must overlap the selected month.
   ...
   Dashboard On Leave = 3
*/

function isEmployeeOnLeave(employeeID, month) {
        ...
        <div class="form-grid">
            ...
            <div class="form-field">
                ...
                >
                    ...
                    <option
                        value="loan"
                        ${transaction.type === "loan" ? "selected" : ""}
                    >
                        Loan Given
                    </option>
            ...
            <div class="form-field full">
                ...
                <textarea
                    ...
                    rows="3"
                >${escapeHTML(transaction.notes || "")}</textarea>
...
if ($("transactionEmployee")) {
    ...
    $("transactionEmployee").addEventListener(
        "change",
        renderTransactions
    );
}
    ...
    openModal(
        ...
        <div class="form-grid">
            ...
            <div class="form-field full">
                ...
                <small>
                    Start Date, End Date and Number of Days
                    are all optional.
...
function addLeave() {
    ...
    /* EMPLOYEE IS THE ONLY REQUIRED FIELD */
    ...
    /* DATES ONLY VALIDATED IF BOTH EXIST */
...
function updateLeave(id) {
    ...
    const employeeId =
        $("editLeaveEmployee").value;

    const startDate =
        $("editLeaveStart").value;
...
function renderDashboard() {
    ...
    /*
       IMPORTANT:
       Count unique employees who are on leave.
    */
    ...
    employees.forEach(employee => {
        ...
        totalPending += details.pending;

        totalFood +=
            getEmployeeFood(employee);

        if (
            details.status === "Fully Paid"
        ) {
            fullyPaid++;
        }
...
function getPrintFilterName(filter) {
    ...
    return names[filter] || "All Employees";
}
